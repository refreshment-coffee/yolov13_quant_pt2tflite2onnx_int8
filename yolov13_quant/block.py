...
class C2f(nn.Module):
    ...
    #     return self.cv2(torch.cat(y, 1))
    def forward(self, x):
        # use split with explicit sizes to generate ONNX Split
        y = self.cv1(x).split((self.c, self.c), 1)  # returns tuple(len=2)
        y = [y[0], y[1]]
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
...
class AAttn(nn.Module):
    ...
    def __init__(self, dim, num_heads, area=1):
        ...
        self.num_heads = num_heads ##default is 4
        self.head_dim = head_dim = dim // num_heads  ##default is 32
    ...
    def forward(self, x):
        ...
        # qk: (B, all_head_dim*2, H, W) -> want (B, num_heads, N, head_dim) for q and k
        qk = self.qk(x)           # (B, all_head_dim*2, H, W)
        v = self.v(x)             # (B, all_head_dim, H, W)
        pp = self.pe(self.v(x))   # pe path (B, dim, H, W)  (we will add after proj)

        # reshape to (B, num_heads, head_dim, N) then permute to (B, num_heads, N, head_dim)
        # use view with explicit dimensions (keeps rank >= 4)
        # first, flatten spatial dims but keep head grouping
        # qk -> (B, 2, num_heads, head_dim, H, W) -> then combine H*W to N

        qk = qk.view(B, 2, self.num_heads, self.head_dim, H, W)
        # split q and k along dim=1 (the 2)
        q = qk[:, 0]   # (B, num_heads, head_dim, H, W)
        k = qk[:, 1]   # same
        # move spatial dims to last and merge: -> (B, num_heads, H*W, head_dim)
        ...
        # v: (B, all_head_dim, H, W) -> (B, num_heads, N, head_dim)
        v = v.view(B, self.num_heads, self.head_dim, H, W)
        v = v.permute(0,1,3,4,2).contiguous().view(B, self.num_heads, N, self.head_dim)

        # ---- batched matmul attention (rank >= 4 throughout) ----
        # q @ k^T : (B, num_heads, N, head_dim) @ (B, num_heads, head_dim, N) -> (B, num_heads, N, N)
        # keep this as batched matmul -> will remain batched in TF/ONNX (not collapsed to FC)
        ...
        # v @ attn^T -> (B, num_heads, N, head_dim)
        out = torch.matmul(attn, v)  # (B, num_heads, N, head_dim)

        # restore to (B, all_head_dim, H, W)
        out = out.view(B, self.num_heads, N, self.head_dim).permute(0,1,3,2).contiguous()
        out = out.view(B, self.num_heads * self.head_dim, H, W)  # (B, all_head_dim, H, W)

        # add positional/pe (pp) and project
        out = self.proj(out + pp)
...
class AdaHyperedgeGen(nn.Module):
    def __init__(self, node_dim, num_hyperedges, num_heads=4, dropout=0.1, context="both"):
        ...
        if context in ("mean", "max"):
            self.context_net = nn.Conv1d(node_dim, num_hyperedges * node_dim, kernel_size=1)
        elif context == "both":
            self.context_net = nn.Conv1d(2 * node_dim, num_hyperedges * node_dim, kernel_size=1)
        else:
            raise ValueError("Unsupported context")

        # ---- 替换 pre_head_proj: Linear → Conv1x1 ----
        # print("use conv1d")
        self.pre_head_proj = nn.Conv1d(node_dim, node_dim, kernel_size=1)
    
        self.dropout = nn.Dropout(dropout)
        # print("self.head_dim: ",self.head_dim)  ##16
    ...
    def forward(self, X):
        ...
        if self.context == "mean":
            ctx = X.mean(dim=1).unsqueeze(-1)
        elif self.context == "max":
            ctx, _ = X.max(dim=1)
            ctx = ctx.unsqueeze(-1)
        else:
            avg = X.mean(dim=1).unsqueeze(-1)
            mx, _ = X.max(dim=1)
            mx = mx.unsqueeze(-1)
            ctx = torch.cat([avg, mx], dim=1)

        # Conv1x1
        proto_offsets = self.context_net(ctx)  # (B, H*D, 1)
        proto_offsets = proto_offsets.view(B, self.num_hyperedges, D)

        prototypes = self.prototype_base.unsqueeze(0) + proto_offsets

        # pre head projection
        X_proj = self.pre_head_proj(X_t).permute(0, 2, 1)

        X_heads = X_proj.view(B, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        ...
        # logits = torch.matmul(X_heads_flat, proto_heads_flat) / self.scaling

        # scale = 1.0 / self.scaling
        # logits = torch.matmul(X_heads_flat, proto_heads_flat)/self.scaling
        logits = torch.matmul(X_heads_flat, proto_heads_flat)
        # logits = torch.div(logits, torch.tensor(self.scaling, dtype=logits.dtype, device=logits.device))

        # scale = torch.tensor(self.scaling, dtype=X_heads_flat.dtype, device=X_heads_flat.device)
        # logits = torch.div(torch.matmul(X_heads_flat, proto_heads_flat), scale)
        
        logits = logits.view(B, self.num_heads, N, self.num_hyperedges).mean(1)
...
class AdaHyperedgeGen_old(nn.Module):
    """
...
class AdaHGConv(nn.Module):
    ...
    def __init__(self, embed_dim, num_hyperedges=16, num_heads=4, dropout=0.1, context="both"):
        ...
        ##use_conv1d
        self.edge_proj_conv1d=nn.Sequential(
            nn.Conv1d(embed_dim, embed_dim, kernel_size=1 ),
            nn.ReLU()
        )
        
        self.node_proj_conv1d=nn.Sequential(
            nn.Conv1d(embed_dim, embed_dim, kernel_size=1 ),
            nn.ReLU()
        )
        ##use_conv1d
        ...
        ##use conv1d  replace nn.Linear
        
    def forward(self, X):
        ##X:B,N,D 
        A = self.edge_generator(X)   ##A:B,N,D2
        
        He = torch.bmm(A.transpose(1, 2), X)   ##B,D2,D
        
        ## use linear 
        # He = self.edge_proj(He)  ### B,D2, D---->B,D2,D 
        ## use linear

        ##use conv1d 
        conv1d_He=He.permute(0,2,1)    ### B,D,D2 
        conv1d_He=self.edge_proj_conv1d(conv1d_He)  ## B,D,D2---> B,D,D2
        He=conv1d_He.permute(0,2,1) ## B,D2,D
        ##use conv1d 
        ...
        ##use conv1d 
        conv1d_X_new=X_new.permute(0,2,1)
        conv1d_X_new=self.node_proj_conv1d(conv1d_X_new)  ## B,D,D2---> B,D,D2
        X_new=conv1d_X_new.permute(0,2,1) ## B,D2,D
        ##use conv1d 
...
class HyperACE(nn.Module):
    ...
    def forward(self, X):
        ...
        y = self.cv1(x).split((self.c, self.c,self.c), 1)  # returns tuple(len=2)
        y = [y[0], y[1],y[2]]
